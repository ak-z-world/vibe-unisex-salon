    ...
    >
      ...
      >
        ...
        >
          <Image
            src="/images/logo.png"
            alt="Vibe Unisex Salon Logo"
            width={180}
            height={80}
            priority
            className={`h-15 md:h-16 w-auto rounded-lg transition-all duration-300 ${scrolled ? "opacity-90" : "opacity-100"
              }`}
          />
        </Link>
        ...
        <ul className="hidden md:flex items-center gap-8" role="list">
          {navLinks.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className={`font-body text-sm tracking-[0.1em] uppercase transition-colors duration-300 ${scrolled
        ...
        {/* Mobile toggle */}
        <button
          className={`md:hidden p-2 transition-colors duration-300 ${scrolled ? "text-[#1A1410]" : "text-white drop-shadow-md"
            }`}
