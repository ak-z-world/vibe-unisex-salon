import { Metadata } from "next";

// Server Component — preserved for SEO crawlability
export default function CitySEOContent() {
  return (
    <section
      className="bg-[#FDFAF6] py-24 border-t border-[#EDE5D8]"
      aria-label="About Vibe Salon Chennai — Detailed Guide"
    >
      <div className="container mx-auto px-6 lg:px-16 max-w-4xl">
        {/* Visual header */}
        <div className="flex items-center gap-4 mb-10">
          <span className="h-px w-10 bg-gradient-to-r from-[#C9A84C] to-[#E7D8B1]" />
          <span className="text-[10px] tracking-[0.38em] uppercase text-[#9A8060] font-medium">
            Discover Vibe Salon
          </span>
        </div>

        <div className="space-y-12 text-[#7A6A58] font-light leading-relaxed text-[15px]">

          {/* Section 1 */}
          <div>
            <h2 className="font-display text-3xl text-[#2C2117] mb-5 font-normal">
              Best Unisex Salon in Chennai
            </h2>
            <p className="mb-4">
              When it comes to finding the best unisex salon in Chennai, Vibe Unisex Salon stands
              in a class of its own. Since 2018, we have been serving thousands of discerning
              clients across Chennai with premium-grade hair, beauty, and grooming services — all
              delivered from elegantly designed salon spaces that reflect the international
              standards our brand embodies.
            </p>
            <p className="mb-4">
              Unlike neighbourhood salons, Vibe approaches every client interaction as an
              experience. From the moment you walk in, you&apos;re welcomed into a refined
              environment where expert stylists give you their complete attention. We listen,
              assess, consult, and then execute — ensuring that every service you receive is
              tailored precisely to your hair type, face shape, skin tone, and personal aesthetic.
            </p>
            <p>
              With over 15,000 satisfied clients and a consistent 4.9-star rating, Vibe Unisex
              Salon has earned its reputation as Chennai&apos;s most trusted premium salon brand —
              not through aggressive marketing, but through excellence, consistency, and care
              delivered one client at a time.
            </p>
          </div>

          {/* Section 2 */}
          <div>
            <h2 className="font-display text-3xl text-[#2C2117] mb-5 font-normal">
              Luxury Hair Salon Services in Chennai
            </h2>
            <p className="mb-4">
              Vibe Salon offers a comprehensive suite of luxury hair salon services in Chennai,
              covering everything from precision haircuts and professional hair coloring to advanced
              treatments like keratin smoothening, hair spas, and scalp therapies. Every service is
              executed using globally acclaimed professional products from brands including
              L&apos;Oréal Professionnel, Wella Professionals, and Kérastase.
            </p>
            <p className="mb-4">
              Our hair coloring services are among the most sought-after in Chennai. Whether you
              want a natural balayage, bold ombre, rich global color, or fashion highlights, our
              Wella-certified colorists bring both artistry and technical expertise to every strand.
              We follow international color science protocols to minimize damage and maximize
              vibrancy, giving you hair color that looks stunning from day one and holds beautifully
              for weeks.
            </p>
            <p>
              For clients seeking hair smoothening in Chennai, our keratin treatments have
              transformed thousands of frizz-prone or difficult-to-manage hair textures. These
              treatments seal the hair cuticle, eliminate frizz, and add a mirror-like shine — with
              results that last three to six months depending on hair type and post-care routine.
              Our stylists provide detailed guidance on how to maintain your results between salon
              visits.
            </p>
          </div>

          {/* Section 3 */}
          <div>
            <h2 className="font-display text-3xl text-[#2C2117] mb-5 font-normal">
              Professional Bridal Makeup Services in Chennai
            </h2>
            <p className="mb-4">
              Vibe Salon is one of Chennai&apos;s most respected destinations for bridal makeup.
              Our certified bridal artists specialize in creating looks that are both timeless and
              personalized — from traditional South Indian bridal aesthetics with heavy silk and
              temple jewellery to contemporary, editorial bridal styles for the modern bride.
            </p>
            <p>
              We recommend that brides-to-be schedule a consultation and trial session at least
              four to six weeks before the wedding. This allows our artists to understand your
              preferences, skin undertone, venue lighting, outfit palette, and photographic
              requirements — so the final look on your wedding day is exactly what you envisioned,
              if not more beautiful. Our bridal packages in Chennai can include hair styling,
              pre-bridal facial treatments, and complete event day coordination.
            </p>
          </div>

          {/* Section 4 */}
          <div>
            <h2 className="font-display text-3xl text-[#2C2117] mb-5 font-normal">
              Hair Spa and Hair Treatments in Chennai
            </h2>
            <p className="mb-4">
              Regular hair spa treatments are the foundation of long-term hair health — and at Vibe
              Salon, we elevate this ritual to a luxury experience. Our hair spa services in Chennai
              address a wide range of concerns including dryness, breakage, scalp irritation, hair
              thinning, and post-chemical damage recovery.
            </p>
            <p className="mb-4">
              Each session begins with a thorough hair and scalp assessment. Based on your unique
              profile, our stylists recommend the most effective treatment from our menu of
              restorative spa protocols. Using Kérastase and L&apos;Oréal Professionnel treatment
              lines, we deliver targeted nourishment, strengthening, and hydration — leaving hair
              visibly transformed. Clients consistently report softer, shinier, and noticeably
              healthier hair after just one session.
            </p>
            <p>
              Beyond hair spas, Vibe also offers scalp detox treatments, protein hair masks,
              bond-building treatments for chemically compromised hair, and customized deep
              conditioning rituals — making us the complete destination for hair care in Chennai.
            </p>
          </div>

          {/* Section 5 */}
          <div>
            <h2 className="font-display text-3xl text-[#2C2117] mb-5 font-normal">
              Men&apos;s Grooming Services at Vibe Salon Chennai
            </h2>
            <p className="mb-4">
              The modern Chennai man understands that personal grooming is not an indulgence — it
              is an investment. Vibe Unisex Salon&apos;s men&apos;s grooming suite is designed to
              meet this need with precision and sophistication.
            </p>
            <p className="mb-4">
              Our men&apos;s grooming services in Chennai include precision haircuts styled to your
              face structure and lifestyle, detailed beard shaping and sculpting, scalp treatments
              for dandruff or thinning hair, skin-specific facial services, and professional hair
              coloring for men. Every service is delivered in our relaxed, private-feeling salon
              environment by stylists trained specifically in men&apos;s grooming techniques.
            </p>
            <p>
              Whether you&apos;re preparing for a board meeting, a wedding, a photoshoot, or simply
              want to maintain a polished everyday look, Vibe&apos;s men&apos;s grooming specialists
              ensure you leave looking and feeling your best — every single visit.
            </p>
          </div>

          {/* Section 6 */}
          <div>
            <h2 className="font-display text-3xl text-[#2C2117] mb-5 font-normal">
              Why Chennai Clients Choose Vibe Salon
            </h2>
            <p className="mb-4">
              Clients across Chennai choose Vibe Salon for three core reasons: expertise,
              consistency, and the experience. Our team of certified stylists, colorists, and
              beauty artists undergoes continuous training and stays updated with international
              beauty trends and techniques. The result is a salon team that can execute everything
              from the most classic cuts to the most complex color transformations — with equal
              confidence.
            </p>
            <p className="mb-4">
              Consistency is the hallmark of a truly premium salon brand. At Vibe, we have built
              systems and training standards that ensure you receive the same elevated experience
              across all five of our Chennai branches. The products, protocols, hygiene standards,
              and service philosophy are uniform — so you are never disappointed, regardless of
              which location you visit.
            </p>
            <p>
              Finally, the Vibe experience is something our clients describe as genuinely different.
              The ambiance, the attention to detail, the way our stylists engage and consult — it all
              adds up to a visit that feels more like a luxury escape than a quick salon errand.
              That&apos;s the experience we have built, and it&apos;s the reason our clients keep
              coming back — and keep referring their family and friends to Vibe Unisex Salon,
              Chennai&apos;s finest premium salon brand.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}

export const citySEOMetadata: Partial<Metadata> = {
  keywords: [
    "Best Unisex Salon in Chennai",
    "Premium Salon Chennai",
    "Luxury Salon Chennai",
    "Hair Salon Chennai",
    "Beauty Salon Chennai",
    "Hair Cut Chennai",
    "Hair Spa Chennai",
    "Keratin Treatment Chennai",
    "Hair Coloring Chennai",
    "Bridal Makeup Chennai",
    "Facial Treatment Chennai",
    "Mens Grooming Chennai",
    "Womens Beauty Services Chennai",
    "Professional Hairstylist Chennai",
    "Hair Smoothening Chennai",
    "Hair Care Chennai",
    "Best hair salon near me in Chennai",
    "Luxury unisex salon in Chennai",
    "Premium hair styling salon Chennai",
    "Best bridal makeup artist Chennai",
    "Best keratin treatment salon Chennai",
    "Hair spa and facial packages Chennai",
  ],
};