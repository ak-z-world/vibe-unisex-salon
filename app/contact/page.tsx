...
// Omit email from the form data type locally if you haven't updated the schema file yet
type WhatsAppFormData = Omit<ContactFormData, "email">;
...
function SuccessScreen({ branchName, onReset }: { branchName: string; onReset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-8 text-center animate-fadeIn">
      ...
      <p className="text-[10px] tracking-widest uppercase text-[#c9a96e] mb-3">Redirected to WhatsApp</p>
      ...
      <p className="text-sm text-[#8a7d6b] leading-relaxed max-w-xs mb-8">
        We have routed you to the WhatsApp chat for <span className="text-[#c9a96e]">{branchName}</span>. Send the generated message to confirm your slot!
      </p>
      <button
        onClick={onReset}
...
export default function ContactPage() {
  ...
  const onSubmit = (data: WhatsAppFormData) => {
    ...
    try {
      ...
      // Default to adding Indian country code (91) if it's purely a 10-digit number.
      const whatsappNumber = cleanPhone.length === 10 ? `91${cleanPhone}` : cleanPhone;

      // 2. Format the message payload
      const textMessage = `*New Appointment Enquiry* ✂️\n\n*Name:* ${data.fullName}\n*Mobile:* ${data.mobile}\n*Service:* ${data.service}\n*Date:* ${data.preferredDate}\n*Time:* ${data.preferredTime}${
        data.message ? `\n*Special Request:* ${data.message}` : ""
      }\n\n_Routed to: ${selectedBranch.name}_`;
            ...
            <div>
              <div className="bg-[#110e09] border border-[#1e1a12] rounded-sm p-6 md:p-8">
                ...
                <p className="text-xs text-[#4a4030] mb-6">
                  Select the branch you'd like to visit to connect with them on WhatsApp.
                </p>
              ...
              <div className="mt-8 space-y-3">
                {[
                  ...
                  { q: "How quickly will you respond?", a: "Our team will text you back on WhatsApp within minutes during working hours." },
                ].map((item, i) => (
                  <details
                    key={i}
            ...
            <div className="bg-[#110e09] border border-[#1e1a12] rounded-sm overflow-hidden">
              {submitState === "success" ? (
                ...
                <form
                  ...
                  noValidate
                  aria-label="WhatsApp Appointment enquiry form"
                ...
                >
                  ...
                  {/* Mobile */}
                  <div>
                    <Label htmlFor="mobile" required>WhatsApp Number</Label>
                    <input
                      id="mobile"
                      type="tel"
                      inputMode="tel"
                  ...
                  {!selectedBranch && (
                    <p className="text-[11px] text-[#5a5040] italic">
                      ⚠ Please select a branch above before continuing to WhatsApp.
                    </p>
                  )}
